...
# .dependencies["com.devgame.editor-presence"] — presence of the KEY means
...
| ----------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **S1**                                                | `unity` not on PATH, not found at `~/.unity/bin/unity` either                                                               | _"Unity's command-line tool isn't installed on this machine. DevGame needs it to talk to the Editor."_                                                   | The app's S1 message now carries the remedy with a copy button: `brew install --cask unity-cli` — VERIFIED 2026-08-11: the cask installs cleanly and links `unity` onto PATH at `/opt/homebrew/bin/unity` (no sudo prompts, no rc-file edits needed with a standard Homebrew setup). Non-macOS: point at Unity's own CLI docs (https://docs.unity.com/en-us/unity-cli).                                                                                                                                                                                                          |
...
| **S3** _(pre-write gate, not a standalone diagnosis)_ | before attempting ANY write below, run `unity auth status --json`                                                           | _"You're not signed in to the Unity CLI."_                                                                                                               | `unity auth login` opens a browser OAuth flow — it must be run by the user, in their own terminal. Print the exact command; do not attempt to run it yourself. VERIFIED 2026-08-11: `unity pipeline install` succeeds with a fully signed-out CLI (clean-identity probe), so login is NOT required for the Setup Integrations path — never gate on auth; only surface it if a write actually fails on an auth error (e.g. Unity cloud features).                                                                                                                                 |
