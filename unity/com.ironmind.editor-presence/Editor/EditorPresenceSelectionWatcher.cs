...
//
// SEQ PERSISTENCE: `_sequence` is backed by UnityEditor.SessionState, not a
// plain static field. A plain static int resets to 0 on every domain
// reload (script recompile — this project's own EditorSettings.asset also
// fires one on every Play press, though this thin plugin no longer touches
// Play/Stop itself) while EditorPresenceConnection.SessionId — ALSO
// SessionState-backed, see that file — stays the same across the reload.
// If seq resets to 1 while the server still holds a stale high-water-mark
// seq (say 50) for that unchanged session.id, the registry's
// `existing.selection.seq <= existing.selection.seq` guard
