...
export const T3ProjectFileScript = Schema.Struct({
  name: trimmedNonEmpty({
    description: "Display name for the script, shown in the DevGame scripts menu.",
  }),
  command: trimmedNonEmpty({
    description: "Shell command executed in a DevGame terminal at the project root.",
  }),
  ...
  description: "A project script that team members can import into DevGame.",
});
...
export const T3ProjectFile = Schema.Struct({
  ...
  iconPath: Schema.optionalKey(
    trimmedNonEmpty(
      {
        description:
          'Workspace-relative path to the project icon (e.g. "assets/logo.svg"). Checked before DevGame\'s built-in icon locations.',
      },
  ...
  scripts: Schema.optionalKey(
    Schema.Array(T3ProjectFileScript)
      .annotate({
        description: "Project scripts shared with everyone who opens this repository in DevGame.",
      })
