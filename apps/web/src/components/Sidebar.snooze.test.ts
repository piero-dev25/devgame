...
describe("resolveSnoozePresets", () => {
  ...
  it("formats preset times with the selected clock preference", () => {
    ...
    // Locale-agnostic 12-hour assertion: upstream's /PM/i assumes en-US ICU
    // output ("PM"), but the formatter follows the machine's default locale
    // and e.g. en-CA renders "p.m." (the dot breaks the match). 18:00
    // rendering with hour 6 IS the 12-hour claim, in every locale.
    expect(twelveHour.find((preset) => preset.id === "evening")!.whenLabel).toContain("6:00");
...
describe("snoozeWakeDescription", () => {
  ...
  it("formats wake descriptions with the selected clock preference", () => {
    // Same locale-agnostic form as the preset test above.
    expect(
      snoozeWakeDescription(localDate(2026, 4, 8, 18).toISOString(), now, "12-hour"),
    ).toContain("6:00");
