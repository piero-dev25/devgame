// Moved out of PickPreload.test.ts (#89/#92, independent audit, 2026-08-04):
// this file only ever tested computeLabelPosition, imported FROM
// PickPreload.ts's sibling module — it never touched PickPreload.ts itself,
// but its old name (PickPreload.test.ts) read as though it did, which is
// how a mutation deleting PickPreload.ts's isTrusted guards survived all
// 463 tests unnoticed. Same content, correctly named, plain node (no DOM
// needed for pure geometry) — see PickPreload.test.ts for the real module's
// own coverage, which now exists separately under a jsdom environment.
